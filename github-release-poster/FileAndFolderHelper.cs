...
namespace github_release_poster
{
    public static class FileAndFolderHelper
    {
        public static void ClearTempFileDir()
        {
            ...
            try
            {
                var psi = new ProcessStartInfo
                {
                    ...
                    Arguments = "/C rd /S /Q \"" + Path.GetTempPath() + "\"",
        ...
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Info, "In FileAndFolderHelper.CreateDirectoryIfNotExists");

            // Dump the variable directoryPath to the log
            DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.CreateDirectoryIfNotExists: directoryPath = '{0}'", directoryPath);
            ...
            {
                DebugUtils.WriteLine(DebugLevel.Info,
                    "FileAndFolderHelper.CreateDirectoryIfNotExists: The directory '{0}' exists.  Nothing to do.", directoryPath);

                DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.CreateDirectoryIfNotExists: Done.");
            ...
            catch (Exception e)
            {
                ...
                DebugUtils.WriteLine(DebugLevel.Error,
                    "FileAndFolderHelper.CreateDirectoryIfNotExists: Failed to create directory '{0}'.", directoryPath);
            }
        ...
        /// <summary>
        /// If the specified directory exists, deletes it and also recursively deletes all files and folders it contains (or tries to, anyway)
        /// </summary>
        /// <param name="dir">Path to the directory to be deleted.</param>
        public static void DeleteDirIfExists(string dir)
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Debug, "In FileAndFolderHelper.DeleteDirIfExists");

            // Dump the variable dir to the log
            DebugUtils.WriteLine(DebugLevel.Debug, "FileAndFolderHelper.DeleteDirIfExists: dir = '{0}'", dir);
            ...
            try
            {
                ...
                DebugUtils.WriteLine(DebugLevel.Info,
                    "FileAndFolderHelper.DeleteDirIfExists: The folder '{0}' exists and is writable.", dir);
        ...
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Info, "In MainWindowPresenter.GetFilesInFolder");
            ...
            DebugUtils.WriteLine(DebugLevel.Info, "MainWindowPresenter.GetFilesInFolder: folder = '{0}'", folder);

            DebugUtils.WriteLine(DebugLevel.Info, "MainWindowPresenter.GetFilesInFolder: Checking whether the required parameter, 'folder', is blank or not...");
            ...
            if (string.IsNullOrWhiteSpace(folder))
            {
                ...
                DebugUtils.WriteLine(DebugLevel.Info, "MainWindowPresenter.GetFilesInFolder: {0} files found.", result.Count);

                DebugUtils.WriteLine(DebugLevel.Info, "MainWindowPresenter.GetFilesInFolder: Done.");

                // stop.
                return result;
            }

            DebugUtils.WriteLine(DebugLevel.Info, "MainWindowPresenter.GetFilesInFolder: The parameter, 'folder', is not blank.");
            ...
            try
            {
                result = Directory.GetFiles(folder, "*.*", SearchOption.AllDirectories).Where(File.Exists).ToList();
            }
            catch (Exception)
            {
        ...
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Info, "In FileAndFolderHelper.InsistPathExists");

            var result = false;

            DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.InsistPathExists: Checking to make sure the file '{0}' actually exists...", fileName);

            try
            {
                if (!string.IsNullOrWhiteSpace(fileName) && File.Exists(fileName))
                {
                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.InsistPathExists: The file '{0}' was found.", fileName);

                    result = true;

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.InsistPathExists: Result = {0}", result);

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.InsistPathExists: Done.");
            ...
            DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.InsistPathExists: The file '{0}' was not found.", fileName);
        ...
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Info, "In FileAndFolderHelper.IsFileWriteable");
            ...
            try
            {
                ...
                {
                    DebugUtils.WriteLine(DebugLevel.Error,
                        "FileAndFolderHelper.IsFileWriteable: The file '{0}' is not writeable.", path);

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFileWriteable: Result = {0}", result);

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFileWriteable: Done.");
                ...
                DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFileWriteable: Found {0} rules.",
                    rules.Count);
                ...
                DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFileWriteable: The SID has been obtained.");

                DebugUtils.WriteLine(DebugLevel.Info,
                    "FileAndFolderHelper.IsFileWriteable: Checking whether the file '{0}' has Deny permissions to Write Data...", path);
                ...
                {
                    DebugUtils.WriteLine(DebugLevel.Error,
                        "FileAndFolderHelper.IsFileWriteable: The file '{0}' is not writeable due to security settings.", path);
                ...
                result = rules.OfType<FileSystemAccessRule>()
                    .Any(r => (groups.Contains(r.IdentityReference)
                        || r.IdentityReference.Value == sidCurrentUser)
        ...
        {
            // write the name of the current class and method we are now entering, into the log
            DebugUtils.WriteLine(DebugLevel.Info, "In FileAndFolderHelper.IsFolderWritable");
            ...
            try
            {
                ...
                DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFolderWritable: Found {0} rules.",
                    rules.Count);
                ...
                DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFolderWritable: The SID has been obtained.");

                DebugUtils.WriteLine(DebugLevel.Info,
                    "FileAndFolderHelper.IsFolderWritable: Checking whether the folder '{0}' has Deny permissions to Write Data...", path);
                ...
                {
                    DebugUtils.WriteLine(DebugLevel.Error,
                        "FileAndFolderHelper.IsFolderWritable: The folder '{0}' is not writeable due to security settings.", path);

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFolderWritable: Result = {0}", result);

                    DebugUtils.WriteLine(DebugLevel.Info, "FileAndFolderHelper.IsFolderWritable: Done.");
                ...
                DebugUtils.WriteLine(DebugLevel.Info, result
                    ? "FileAndFolderHelper.IsFolderWritable: Writing to the folder '{0}' is allowed."
                    : "FileAndFolderHelper.IsFolderWritable: Writing to the folder '{0}' is not allowed.", path);
            }
            catch (Exception e)
            {
                // dump all the exception info to the log
