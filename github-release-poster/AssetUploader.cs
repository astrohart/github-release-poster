...
namespace github_release_poster
{
    ...
    {
        /// <summary>
        /// Uploads the assets in the release asset directory individually to the
        /// newly-created release.   Flattens any directory structure that might exist in
        /// the folder referred to by <see cref="releaseAssetDir" />.
        /// </summary>
        /// <param name="releaseAssetDir">
        /// String containing the fully-qualified path to the
        /// directory containing release assets to be uploaded.
        /// </param>
        /// <param name="newRelease">
        /// Reference to an instance of a dynamic object which has
        /// been deserialized from the response JSON of the call to create the new release.
        /// </param>
        /// <param name="userAccessToken">String containing the user's API access token.</param>
        public static void UploadAssetsIndividually(
        ...
        )
        {
            ...
            if (!Directory.Exists(releaseAssetDir))
                throw new DirectoryNotFoundException(
                    string.Format(Resources.DirectoryNotFound, releaseAssetDir)
                );
            ...
            foreach (var assetFile in FileSearcher
                                      .GetAllFilesInFolder(releaseAssetDir)
                                      .Where(
                                          fsi => (fsi.Attributes &
                                      ...
                                      ))
            {
                ...
                if (string.IsNullOrWhiteSpace(uploadUrl))
                {
                    ...
                    Environment.Exit(
                        Resources.ERROR_NOT_OBTAINED_RELEASE_UPLOAD_URL
                    );
                }
                ...
                var assetMimeMapping =
                    MimeMapping.GetMimeMapping(assetFile.FullName);
                request.AddHeader(
                    Resources.ContentTypeHeaderName, assetMimeMapping
                );
                ...
                Console.WriteLine(
                    $"{bytes.Length} bytes read from file '{assetFile.FullName}'. Uploading..."
                );
                ...
                {
                    Console.WriteLine(
                        Resources.FailedToUploadAsset, assetFileName
                    );
        ...
        /// <summary>
        /// Uploads assets to the release, ZIPping the file tree in the release
        /// directory and then uploading the zip.
        /// </summary>
        /// <param name="releaseAssetDir">
        /// String containing the fully-qualified path to the
        /// directory containing the release's assets.
        /// </param>
        /// <param name="newRelease">
        /// Reference to an instance of a dynamic object
        /// deserialized from the JSON response to the create-new-release action.
        /// </param>
        /// <param name="userAccessToken">String containing the user's API access token.</param>
        /// <param name="assetsZipName">
        /// Name to call the new zip file of assets.  Default
        /// is "assets.zip".
        /// </param>
        /// <returns>True if the upload operation succeeded; false otherwise.</returns>
        /// <exception cref="T:System.ArgumentNullException">
        /// Thrown if any of the
        /// parameters of this method are null references or blank strings.
        /// </exception>
        /// <exception cref="T:System.IO.DirectoryNotFoundException">
        /// Thrown if the release
        /// asset directory cannot be located.
        /// </exception>
        public static void UploadAssetsZipped(
            string releaseAssetDir,
            dynamic newRelease,
            string userAccessToken,
        ...
        )
        {
            ...
            var outputZipFilePath =
                $@"{Path.GetTempPath()}\{Guid.NewGuid()}\{assetsZipName}";

            var outputZipFileName = Path.GetFileName(outputZipFilePath);

            if (!ZipperUpper.CompressDirectory(
                    releaseAssetDir, outputZipFilePath
                ))
            {
            ...
            if (Convert.ToUInt64(new FileInfo(outputZipFilePath).Length) >=
                Resources.TwoGigaBytes)
            {
            ...
            {
                Console.WriteLine(
                    Resources.AssetZipProcessResultedInZeroSizePackage
                );
                Environment.Exit(Resources.ERROR_FAILED_TO_ZIP_ASSETS);
            }
            ...
            request.AddHeader(
                Resources.ContentTypeHeaderName, Resources.ZipFileContentType
            );

            request.AddParameter(
                Resources.ZipFileContentType, fileBytes,
            ...
            {
                Console.WriteLine(
                    Resources.FailedToUploadAsset, outputZipFileName
                );
                Environment.Exit(Resources.ERROR_ASSET_NOT_ACCEPTED);
            }
            else
